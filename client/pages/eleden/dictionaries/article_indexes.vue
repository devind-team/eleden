...
export default defineComponent({
  ...
  setup (props) {
    ...
    useNuxt2Meta({ title: t('dictionaries.articleIndexes.header') as string })

    const bc: ComputedRef<BreadCrumbsItem[]> = computed<BreadCrumbsItem[]>(() => ([
      ...props.breadCrumbs,
      {
        text: t('dictionaries.articleIndexes.header') as string,
        to: localePath({ name: 'eleden-dictionaries-article_indexes' }),
        exact: true
      }
    ]))
