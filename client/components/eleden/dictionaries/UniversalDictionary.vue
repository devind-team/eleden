...
export default defineComponent({
  ...
  setup (props) {
    ...
    const countText = computed<string>(() => {
      ...
      return t('totalCount', { count: result.data.value && result.data.value.length })
    })
    ...
    const tableHeaders = computed<DataTableHeader[]>(() => {
      return props.headers.map((header: Header) => {
        if (typeof header === 'string') {
          return {
            text: t(`dictionaries.${props.queryName}.tableHeaders.${header}`),
            value: header
          }
        }
        return {
          text: t(`dictionaries.${props.queryName}.tableHeaders.${header.name}`),
          value: header.value
        }
      })
    })
