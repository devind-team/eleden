<template lang="pug">
  mutation-modal-form(
    :header="$t('ac.teams.users.addMenu.fromExisting.header')"
    :subheader="team.name + ' (' + team.shortName + ')'"
    :buttonText="$t('ac.teams.users.addMenu.fromExisting.buttonText')"
    ...
    template(#form)
      validation-provider(
        v-slot="{ errors, valid }"
        :name="$t('ac.teams.users.addMenu.form.userId')"
        rules="required"
      )
        v-autocomplete(
          v-model="userId"
